...
const ProfileScreen = () => {
  ...
  if (user) {
    setFormData({
      name: user.name || '',
      bio: userData?.bio || '',
      location: typeof userData?.location === 'object' ? JSON.stringify(userData.location) : (userData?.location || ''),
              ...
              {userData?.location && (
                <View style={styles.profileDetail}>
                  <Text style={styles.profileDetailText}>
                    {typeof userData.location === 'object' && userData.location.coordinates
                      ? `${userData.location.coordinates[0]}, ${userData.location.coordinates[1]}`
                      : userData.location}
                  </Text>
                </View>
              )}
